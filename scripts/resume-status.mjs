...
import {observationBindings} from './observation-bindings.mjs';
...
const now=Date.now();
const observationLedger=observationBindings(run,ledger);
const nextAction=resumeDecision({now,freezeAt:run.freezeAt,deadlineAt:run.deadlineAt,clusterReachable:deployment.ok&&pods.ok,identityMatched:identity.ok,apiReady:api.ok&&api.version===run.deployment.productVersion,observationBindingsMatched:observationLedger.ok});
const report={schemaVersion:1,runId:run.runId,checkedAt:new Date(now).toISOString(),nextAction,freezeAt:run.freezeAt,deadlineAt:run.deadlineAt,head:head.ok?head.stdout:null,workingTree:dirty.ok?dirty.stdout.split('\n').filter(Boolean):null,remoteMain:remote.ok?remote.stdout.split(/\s/)[0]||null:null,remoteGitReachable:remote.ok,clusterReachable:deployment.ok&&pods.ok,api,identity,observationLedger,workingMilestone:ledger.workingMilestone,remainingInOrder:ledger.remainingInOrder,mutationsPerformed:false};
