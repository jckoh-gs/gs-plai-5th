...
(async()=>{
 ...
 const state=await(await fetch(base+'/api/state',{headers,signal:AbortSignal.timeout(15000),redirect:'error'})).json(),plants=state.plants.toSorted((a,b)=>b.controlRuns.length-a.controlRuns.length).slice(0,2);assert.equal(plants.length,2);
 ...
 try{
  ...
  for(const p of plants){
   ...
   const data=JSON.parse(fs.readFileSync(file)),raw=await(await fetch(base+`/api/plants/${p.id}/commands`,{headers,signal:AbortSignal.timeout(15000),redirect:'error'})).json();assert.equal(data.plantId,p.id);assert.equal(data.scope,'recent-command-snapshots');assert(data.commands.length<=20);assert.deepEqual(data.commands.map(c=>c.commandId),raw.slice(0,20).map(c=>c.commandId));
