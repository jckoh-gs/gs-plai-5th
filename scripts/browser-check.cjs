...
(async()=>{
 ...
 const target=new URL(base);let disconnected=false;const sockets=new Set();
 ...
 const proxy=http.createServer((req,res)=>{if(disconnected){res.writeHead(503);res.end();return;}const upstream=http.request({hostname:target.hostname,port:target.port,path:req.url,method:req.method,headers:{...req.headers,host:target.host}},response=>{res.writeHead(response.statusCode,response.headers);response.pipe(res);});upstream.on('error',()=>{if(!res.headersSent)res.writeHead(502);res.end();});res.on('close',()=>upstream.destroy());req.pipe(upstream);});
 ...
 try{
  await page.goto(browserBase);const config=await (await fetch(base+'/api/config')).json();
  ...
  assert.deepEqual(errors,[]);const result={result:'PASS',authRequired:config.authRequired,authenticatedSSE:true,staleAfterFiveSeconds:true,resumeClearsStale:true,guideDownload:true,narrowViewportNoPageOverflow:true,pageErrors:errors};fs.writeFileSync(path.join(directory,'result.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result));
