...
(async()=>{
 ...
 try{
  await page.goto(browserBase);await page.getByText('서버 상태를 불러오는 중입니다…',{exact:true}).waitFor();initial=false;const config=await (await fetch(base+'/api/config',{signal:AbortSignal.timeout(15000),redirect:'error'})).json();
  ...
  assert.deepEqual(errors,[]);const result={result:'PASS',authRequired:config.authRequired,streamReceived:true,authenticatedSSE:config.authRequired?true:null,initialWaitingState:true,openStreamWithoutMessagesStale:true,wrongTokenReentry:config.authRequired?true:null,independentTabRequiresLogin:config.authRequired?true:null,noLocalStorageToken:config.authRequired?true:null,staleAfterFiveSeconds:true,resumeClearsStale:true,guideDownload:true,narrowViewportNoPageOverflow:true,pageErrors:errors};fs.writeFileSync(path.join(directory,'result.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result));
