...
const pod=async()=>JSON.parse(await kubectl(['get','pods','-l','app=grid','-o','json'])).items.find(p=>!p.metadata.deletionTimestamp)?.metadata.uid;
...
try{
 ...
 const backup=`import{DatabaseSync,backup}from'node:sqlite';const db=new DatabaseSync(process.env.DB_PATH,{readOnly:true});await backup(db,'/data/pending-outbox-af7f223.sqlite');db.close();console.log('consistent backup complete');`;
 await kubectl(['exec','deployment/grid','-c','app','--','node','--input-type=module','-e',backup]);
 await kubectl(['rollout','restart','deployment/grid']);await kubectl(['rollout','status','deployment/grid','--timeout=180s']);const afterPod=await pod();assert.notEqual(beforePod,afterPod);
 ...
 const result={result:'PASS',checkedAt:new Date().toISOString(),context:'charles-k3s',namespace:'gs-plai-5h',rtuId:id,beforePod,afterPod,podReplaced:true,messageId:pending.id,sequence:pending.seq,bodySha256:hash(pending.body),samePersistedBodyBeforeAndAfterRestart:true,sameObservedMQTTBody:true,ackedAt:confirmed.acked,consistentPendingBackup:'/data/pending-outbox-af7f223.sqlite'};writeFileSync('deploy/verification/final-af7f223-outbox-restart.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));
