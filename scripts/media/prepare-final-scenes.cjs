...
(async()=>{
 ...
 const c=require('./scene-plan.cjs')({baseUrl:input.baseUrl,tokenFile:input.tokenFile,mqtt:input.mqtt,names,csvFile:input.csvFile,rehearsal:false,...features});
 ...
 const output='artifacts/media-preparation/final-scenes.json';fs.writeFileSync(output,JSON.stringify(c,null,2));console.log(JSON.stringify({output,version:state.version,features}));
