...
function budget({run,rehearsal=false,rehearsalBudgetMs=15*60*1000,now=Date.now,deadline}={}){
 const limit=rehearsal?now()+rehearsalBudgetMs:Date.parse(run.deadlineAt);
...
function supervise(script,{runPath='docs/operations/run.json',rehearsalBudgetMs=15*60*1000}={}){
 ...
 const b=budget({run,rehearsal,rehearsalBudgetMs});b.check();
