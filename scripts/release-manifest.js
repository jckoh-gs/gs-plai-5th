...
const sourceFiles=['package.json','package-lock.json','vite.config.js','Dockerfile',...walk('server'),...walk('web'),...walk('samples'),...walk('scripts'),...walk('tests'),...walk('deploy').filter(p=>!p.startsWith('deploy/verification/')),'docs/protocol.md'];
const sourceHashes=Object.fromEntries(sourceFiles.sort().map(p=>[p,hash(p)]));
const evidenceFiles=['artifacts/checkpoints','deploy/verification','docs/issues/evidence','docs/security/evidence'].flatMap(dir=>existsSync(dir)?walk(dir).filter(p=>/\.(log|json|txt)$/.test(p)):[]);
