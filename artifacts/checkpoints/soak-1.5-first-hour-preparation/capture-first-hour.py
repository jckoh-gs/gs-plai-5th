...
root=Path.cwd();out=root/'artifacts/checkpoints/soak-1.5.0-first-hour'
...
primary=prefix('artifacts/soak/v1.5.0/observations.jsonl')
audit_dir=Path('artifacts/soak/telemetry-audit/2026-09-21T14-56-58-686Z-820d9a15')
...
assert run['deployment']['productVersion']=='1.5.0'
...
for directory in [Path('artifacts/checkpoints/observer-1.5-activation/source'),audit_dir/'source']:
...
summary={'capturedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'sourceHead':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'runtimeCommit':run['deployment']['sourceCommit'],'scope':'Immutable first poll after one hour for each live observer; processes continue. Not uninterrupted uptime or full acceptance proof.','primary':{'sessionId':p['sessionId'],'startedAt':p['startedAt'],'through':p['checkedAt'],'durationSeconds':stamp(p['checkedAt'])-stamp(p['startedAt']),'observations':len(primary),**{k:p[k] for k in ['messages','invalid','duplicates','apiErrors','connectionErrors']},'maxPending':max(h['pending'] for x in primary for h in x.get('health',[])),'nonHealthyPolls':sum(any(h['health']!='HEALTHY' for h in x.get('health',[])) for x in primary),'apiUnavailablePolls':sum(x.get('apiReady') is False for x in primary),'minRssBytes':min(x['rssBytes'] for x in primary if 'rssBytes'in x),'maxRssBytes':max(x['rssBytes'] for x in primary if 'rssBytes'in x)},'supplementary':{'sessionId':a['sessionId'],'startedAt':a['startedAt'],'through':a['checkedAt'],'durationSeconds':stamp(a['checkedAt'])-stamp(a['startedAt']),'observations':len(audit),**{k:a[k] for k in ['received','invalidMessages','duplicateMessages','connectionErrors']},'rtus':[{k:x[k] for k in ['rtuId','samples','invalidMessages','sampleDiscontinuities','baselineDriftMessages','runTransitions']} for x in a['rtus']]},'incidentEvidence':[x['evidence'] for x in resume.get('currentObservationIncidents',[]) if x.get('runtime')=='1.5.0'],'interpretation':'Initial1.5 observercounterbaseline0; allcapturedcounterincrements and known1.5incidentreferences preserved. This is received-observation evidence, not every subscriber receipt or uninterrupted service proof; missingincidentlinks do not prove nooutage.','processesContinue':True}
