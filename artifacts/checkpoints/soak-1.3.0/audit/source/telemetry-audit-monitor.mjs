...
const observerSources = ['scripts/telemetry-audit.mjs', 'scripts/telemetry-audit-monitor.mjs'].map(file => {
  const source = fs.readFileSync(file);
  const savedAs = `source/${path.basename(file)}`;
  fs.writeFileSync(path.join(out, savedAs), source, {flag: 'wx', mode: 0o600});
  return {file, savedAs, sha256: createHash('sha256').update(source).digest('hex')};
});
...
client.on('connect', () => {connected = true; connections++; lastConnectedAt = new Date().toISOString(); client.subscribe('vpp/rtu/+/telemetry', {qos: 1}, (error, granted) => {if (error || !granted?.length || granted.some(item => item.qos > 2)) connectionErrors++;});});
