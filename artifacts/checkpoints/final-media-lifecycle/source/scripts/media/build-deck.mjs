...
const {supervise,workerBudget}=await import('./media-deadline.cjs');
if(!supervise(fileURLToPath(import.meta.url)))await new Promise(()=>{});
...
let factsBytes=null;
if(config.binding){
 const factsPath=await fs.realpath(path.resolve(config.binding.factsFile||'')),publicRoot=await fs.realpath(root),relative=path.relative(publicRoot,factsPath);
 if(!relative||relative==='..'||relative.startsWith('..'+path.sep)||path.isAbsolute(relative)||relative.split(path.sep).some(p=>p==='private'||p==='.env'||p.startsWith('.env.')))throw Error('Reviewed facts must be a public repository file');
 factsBytes=await fs.readFile(factsPath);if(crypto.createHash('sha256').update(factsBytes).digest('hex')!==config.binding.factsSha256)throw Error('Reviewed facts changed after deck binding');
 checkPublicInput(JSON.parse(factsBytes));
}

const rehearsal=process.argv.includes('--rehearsal');
if(!rehearsal&&!config.binding)throw Error('Final deck requires reviewed same-video fact binding');
...
const mediaBudget=workerBudget(run,rehearsal);mediaBudget.check();
const freeze=Date.parse(run.freezeAt),deadline=Date.parse(run.deadlineAt);
if(!rehearsal&&(!Number.isFinite(freeze)||!Number.isFinite(deadline)||freeze>=deadline||Date.now()<freeze||Date.now()>deadline))throw Error('Final deck must occur within frozen final 30 minutes');
...
let reviewedVideo=null;
if(!rehearsal){const video=JSON.parse(await fs.readFile(config.videoVerification,'utf8'));reviewedVideo=video;const digest=value=>typeof value==='string'?value.match(/(?:^|@)(sha256:[a-f0-9]{64})$/)?.[1]:null;const expectedDigest=digest(manifest.deployment?.appImage);if(!expectedDigest||digest(video.imageDigest)!==expectedDigest)throw Error('Video image digest differs from release');const actualVideo=path.resolve(config.outputDir||'artifacts/presentation',videoReference);const actualVideoHash=crypto.createHash('sha256').update(await fs.readFile(actualVideo)).digest('hex');if(!/^[a-f0-9]{64}$/.test(video.videoSha256||'')||actualVideoHash!==video.videoSha256)throw Error('Actual video file differs from reviewed video');if(video.rehearsal||!video.fullDecode||video.releaseCommit!==manifest.source.commit||video.manifestSha256!==manifestHash||video.visualReview!=='passed'||video.claimsReview!=='passed')throw Error('Same-release final video must be completed and reviewed first');if(config.slides.some(s=>!s.notes||!/^\d{2}:\d{2}(?:\s*[–-]\s*\d{2}:\d{2})?$/.test(s.timecode||'')||!s.sources?.length))throw Error('Every slide requires notes, actual MM:SS timecode, and source evidence');if(config.slides.some(s=>/기입합니다|삽입합니다|POPULATE/.test(s.body||'')))throw Error('Unresolved content placeholder');if(config.slides.filter(s=>s.image).length<2||!config.slides.some(s=>s.architecture))throw Error('Actual UI captures and editable architecture are required');}
...
await fs.copyFile(new URL('./media-deadline.cjs',import.meta.url),path.join(sourceDir,'media-deadline.cjs'));
...
const preserved=[];
if(factsBytes){
 await fs.writeFile(path.join(sourceDir,'facts.original.json'),factsBytes);
 await fs.writeFile(path.join(sourceDir,'deck-binding-input.json'),JSON.stringify(config.binding.input,null,2)+'\n');
 preserved.push('source/facts.original.json','source/deck-binding-input.json');
 for(const name of ['prepare-final-deck.cjs','final-deck-binding.cjs','release-features.cjs','validate-command-download.cjs']){
  await fs.copyFile(new URL('./'+name,import.meta.url),path.join(sourceDir,name));preserved.push('source/'+name);
 }
 portable.binding.factsFile='source/facts.original.json';
}
