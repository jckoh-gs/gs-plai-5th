...
export function createTelemetryAudit(plants, {dedupLimit = 10000} = {}) {
  ...
  function receive(topic, bytes, {now = Date.now(), retained = false} = {}) {
    ...
    if (x.schemaVersion !== 2 || !isUuid(x.messageId) || !isUuid(x.runId) || !Number.isSafeInteger(x.sequence) || x.sequence < 1 || !utc(x.createdAt)) add('envelope_contract');
    ...
    for (const f of x.samples) {
      ...
      if (!utc(f.timestamp) || f.time !== f.timestamp || !utc(f.sourceTimestamp) || !(f.weatherObservedAt === null || utc(f.weatherObservedAt))) add('sample_timestamp');
      if (!['csv', 'weather'].includes(f.mode) || !text(f.weatherSource) || !text(f.irradianceSource) || f.controlPath !== 'RTU -> virtual SCADA -> generators') add('sample_provenance');
      if (!record(f.plant) || !text(f.plant.name) || !['wind', 'solar', 'hybrid'].includes(f.plant.type) || !finite(f.plant.lat) || !finite(f.plant.lon) || !Number.isInteger(f.plant.station)) add('plant_fields');
      if (!record(f.weather) || ['wind_speed_ms', 'wind_direction_deg', 'irradiance_wm2', 'temperature'].some(key => !finite(f.weather[key]))) add('weather_fields');
      if (!record(f.electrical) || ['voltage', 'sourceCurrentA', 'estimatedCurrentA'].some(key => !own(f.electrical, key) || !nullableFinite(f.electrical[key])) || !text(f.electrical.currentMethod)) add('electrical_fields');
      if (!record(f.faults) || ['offline', 'sensorFreeze', 'scadaReject', 'actuatorStuck'].some(key => typeof f.faults[key] !== 'boolean') || ['latencyMs', 'dropPct'].some(key => !finite(f.faults[key]))) add('fault_fields');
      ...
      for (const g of f.generators) {
        ...
        if (!record(g) || ['rampKwPerSec', 'limitPct', 'targetKw', 'startupDelaySeconds', 'startupRemaining', 'cutInMs', 'ratedWindMs', 'cutOutMs', 'temperatureCoefficient', 'solarEfficiency'].some(key => !finite(g[key])) || !own(g, 'targetLimitKw') || !nullableFinite(g.targetLimitKw) || typeof g.on !== 'boolean' || typeof g.customWindCurve !== 'boolean' || !['wind', 'solar'].includes(g.type) || !text(g.status)) add('generator_fields');
      }
  ...
  return {
    ...
    snapshot(now = Date.now()) {
      return {received, invalidMessages, duplicateMessages, unknownRtuMessages, reasons: {...totals}, dedupLimit, dedupEntries: seen.size, claimBoundary: 'Received telemetry only. Counts include invalid received messages; observedTwoMessagesAnd60Samples is quantity coverage, not an acceptance verdict. Forward sequence jumps may be command events. Duplicate QoS1 delivery, sample discontinuities, silence, baseline drift and run transitions require context; none alone proves data loss. No pre-subscription coverage.', rtus: [...rtus.values()].map(r => ({...r, generatorIds: [...r.generatorIds], silenceMs: r.lastReceivedAt === null ? null : Math.max(0, now - Date.parse(r.lastReceivedAt)), observedTwoMessagesAnd60Samples: r.uniqueMessages >= 2 && r.samples >= 60}))};
    }
  };
}
