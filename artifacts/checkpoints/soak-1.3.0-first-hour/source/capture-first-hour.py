...
root=Path.cwd();out=root/'artifacts/checkpoints/soak-1.3.0-first-hour'
...
primary=prefix('artifacts/soak/v1.3.0/observations.jsonl')
audit_dir=Path('artifacts/soak/telemetry-audit/2026-09-21T12-22-05-577Z-9a9f5e31')
...
assert run['deployment']['productVersion']=='1.3.0'
for label in ['primary','supplementary','supervisor']:
...
for directory in [Path('artifacts/soak/v1.3.0/source'),audit_dir/'source']:
...
summary={'capturedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'sourceHead':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'runtimeCommit':run['deployment']['sourceCommit'],'scope':'Immutable first poll after one hour for each live observer; processes continue. Not uninterrupted uptime or full acceptance proof.','primary':{'sessionId':p['sessionId'],'startedAt':p['startedAt'],'through':p['checkedAt'],'durationSeconds':stamp(p['checkedAt'])-stamp(p['startedAt']),'observations':len(primary),**{k:p[k] for k in ['messages','invalid','duplicates','apiErrors','connectionErrors']},'maxPending':max(h['pending'] for x in primary for h in x.get('health',[])),'nonHealthyPolls':sum(any(h['health']!='HEALTHY' for h in x.get('health',[])) for x in primary),'apiUnavailablePolls':sum(x.get('apiReady') is False for x in primary),'minRssBytes':min(x['rssBytes'] for x in primary if 'rssBytes'in x),'maxRssBytes':max(x['rssBytes'] for x in primary if 'rssBytes'in x)},'supplementary':{'sessionId':a['sessionId'],'startedAt':a['startedAt'],'through':a['checkedAt'],'durationSeconds':stamp(a['checkedAt'])-stamp(a['startedAt']),'observations':len(audit),**{k:a[k] for k in ['received','invalidMessages','duplicateMessages','connectionErrors']},'rtus':[{k:x[k] for k in ['rtuId','samples','invalidMessages','sampleDiscontinuities','baselineDriftMessages','runTransitions']} for x in a['rtus']]},'incidentEvidence':['artifacts/checkpoints/reconnect-20260921T1303/summary.json','artifacts/checkpoints/reconnect-20260921T1308/summary.json'],'interpretation':'Local observer/API interruptions occurred. Earlier9batch/300sample observed gap has corresponding continuous remote outbox/PUBACK proof; this does not establish every subscriber receipt. Assess any subsequent counter increase separately.','processesContinue':True}
