...
p=pathlib.Path('artifacts/private/media-weather-r2');o=pathlib.Path('artifacts/media-preparation/weather-r2-focused');token=(p/'api-token').read_text().strip();name=json.loads((o/'config.json').read_text())['lifecycle']['registration']['name'];end=time.monotonic()+360
