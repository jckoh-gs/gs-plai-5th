...
for(const name of ['GRID-VPP-presentation-ko.pptx','speaker-notes.md','validation.json','reproduction-manifest.json','source/config.original.json','source/config.portable.json','source/build-deck.mjs','source/media-deadline.cjs','source/REGENERATE.md'])await file(join(deckDir,name));
...
for(const [base,p,key]of [[deckDir,'reproduction-manifest.json','files'],[videoDir,'source/reproduction.json','files']]){
 const data=await json(join(base,p));if(data){
  ...
  if(base===deckDir){
   ...
   const required=['source/config.original.json','source/config.portable.json','source/build-deck.mjs','source/media-deadline.cjs','source/release-manifest.json','source/video-verification.json','docs/operations/run.json','validation.json',...(config?.slides||[]).flatMap((s,i)=>s.image?[`source/assets/slide-${String(i+1).padStart(2,'0')}.png`]:[])];
