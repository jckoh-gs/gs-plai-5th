...
// Demonstrate manual weather only on the newly registered, owned hybrid RTU.
const weatherAt=c.scenes[2].actions.findIndex(a=>a.type==='range'&&a.selector==='#limit');
c.scenes[2].actions.splice(weatherAt,0,
 {type:'focus',label:'발전 입력 모드'},{type:'capture',name:'weather-before'},select('발전 입력 모드','weather'),fill('풍속 (m/s)',14),fill('풍향 (°)',90),fill('일사 (W/m²)',300),fill('온도 (°C)',22),click('기상 적용'),wait(10000),{type:'focus',selector:'.scene canvas'},{type:'capture',name:'weather-model-output'},
 select('발전 입력 모드','csv'),fill('풍속 (m/s)',8),fill('풍향 (°)',240),fill('일사 (W/m²)',650),fill('온도 (°C)',22),click('기상 적용'),wait(12000),{type:'focus',selector:'.scene canvas'},{type:'capture',name:'weather-csv-restored'});
c.scenes[2].seconds+=36;
// Keep both edge-aligned metric columns visible in encoded 1080p frames.
delete c.scenes[3].zoom;
...
if(commandReceipt){c.scenes[2].actions.push(wait(1500),click('저장 상태 확인'),wait(1500),{type:'capture',name:'rest-receipt-stored-status'});c.scenes[2].seconds+=8;c.scenes[2].narration+=' 명령 접수는 완료와 다릅니다. 접수 패널은 마지막으로 받은 결과를 보여주며, 저장 상태 확인으로 서버 상태를 다시 조회합니다.';}
if(commandExport){delete c.scenes[4].zoom;c.scenes[4].actions.push({type:'download',role:'button',name:'최근 명령 상태 JSON 내보내기',file:'recent-command-states.json',validate:'command-states',rtuId:'selected'},wait(1500),{type:'capture',name:'command-export'});c.scenes[4].narration+=' 최근 저장 상태를 최대 스무 개까지 제이슨 파일로 내보냅니다. 이 파일은 외부 VPP 수신 증거나 전체 감사 이력이 아닙니다.';}
if(lifecycle){
 c.lifecycle=lifecycle;
 const mutating=new Set(['기상 적용','출력 상한 적용','장애 적용','모든 장애 해제','재생 설정 적용','일시정지','재생','현재 상태 저장']);
 for(const scene of c.scenes)for(const action of scene.actions){
  if(action.type==='click'&&action.name==='RTU 등록 · 수집 시작')action.type='register';
  else if(action.type==='click'&&action.selector?.startsWith('.scenario:')){action.type='restoreScenario';action.scenarioName=names.scenario;action.requiresOwned=true;}
  else if(action.type==='dispatch'||(action.type==='click'&&(mutating.has(action.name)||action.selector?.startsWith('.dashboard-bottom '))))action.requiresOwned=true;
 }
}
return c;
};
// Confirm the actual submitted form matches durable ownership intent before
// accepting a correlated registration response as a new demo RTU baseline.
module.exports.validateRegistrationRequest=function(body,spec){
 const fields=['name','type','count','ratedKw','rampKwPerSec'];
 if(!body||typeof body!=='object'||Array.isArray(body)||!spec||fields.some(key=>body[key]!==spec[key]))throw Error('Actual registration request differs from demo ownership intent');
 return true;
};
