...
p=pathlib.Path('deploy/verification/candidate-d98d3c4/outbox-observation');k=['kubectl','--context','charles-k3s','-n','gs-plai-5h'];start=time.monotonic();end=min(start+120,start+max(0,datetime.datetime.fromisoformat(json.load(open('docs/operations/run.json'))['deadlineAt']).timestamp()-time.time()));rows=[]
while time.monotonic()<end:
 ...
 if proof.exists() and json.loads(proof.read_text()).get('result')=='PASS' and row['health'].get('status')=='ok' and all(c['ready'] for x in row.get('pods',[]) for c in x['containers']):
...
(p/'summary.json').write_text(json.dumps({'result':'OBSERVED','readOnly':True,'startedAt':rows[0]['at']if rows else None,'endedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'samples':len(rows),'podUIDs':sorted({x['uid']for r in rows for x in r.get('pods',[])}),'localHealthErrors':sum('errorType'in r['health']for r in rows),'stopReason':'root-confirmed-terminal'if(p/'stop-request.json').exists()else'bounded120seconds','limitation':'Observation begins after delegation; may miss earlier planned replacement. Root operation proof establishes full transition.'},indent=2)+'\n')
