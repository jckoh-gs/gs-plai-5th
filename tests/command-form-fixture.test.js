...
test('unreviewed product or target modes fail closed',()=>{
 for(const version of ['1.5.1','1.5.0-rc.1','1.6.1','1.7.2','1.8.0'])assert.throws(()=>fixtureConfig({GRID_CANDIDATE_VERSION:version,EXPECTED_BUNDLE_SHA256:hash}),/Unreviewed/);
...
for(const version of ['1.7.0','1.7.1'])test('approved'+version+' form fixture is isolated3111 with an explicit candidate bundle',()=>{
 const env={GRID_CANDIDATE_VERSION:version,EXPECTED_BUNDLE_SHA256:hash};const c=fixtureConfig(env);assert.equal(c.base,'http://127.0.0.1:3111');assert.equal(c.prefix,'candidate17');assert.equal(c.tokenFile,'artifacts/private/candidate-1.7/api-token');assert.throws(()=>fixtureConfig({...env,EXPECTED_BUNDLE_SHA256:undefined}),/bundle SHA256/);assert.throws(()=>fixtureConfig({...env,GRID_URL:'http://127.0.0.1:3110'}),/mismatch/);assert.equal(fixtureConfig({...env,GRID_FIXTURE_TARGET:'isolated-restore'}).base,'http://127.0.0.1:3105');
});
