...
문서 버전: 1.16 · 기준일: 2026-09-22 · 제품 기준: SCADA Emulation Engine v2 / MQTT 계약 v2
...
IDEA010 개정1 / 제품1.6.0 후보: 시각은 서버 이벤트의 canonical `created` UTC ISO 값만 사용한다. 실제 달력 날짜와 유한 시각을 검증한 뒤 **Asia/Seoul(KST)**로 표시하고 시간대를 화면에 명시한다. 원본 UTC ISO는 접근 가능한 보조 정보로 제공한다. 잘못된 값·누락은 대시로 표시하고 현재시각이나 다른 필드로 추정하지 않으며 원본 잘못된 값도 그대로 노출하지 않는다. 서버·DB·API 및 전역 최근80개/id내림차순·유형·메시지 계약은 유지한다. 이 수정은 쓰기·검색·필터·export를 추가하지 않는다. stable1.5의 ISSUE023 시각 누락은 알려진 결함이며 기존 메시지/열 표시 검증을 실제 시각 정확성 검증으로 확대하지 않는다.

#### AT-EVENT-TIME-01 인수 — 구현·검증 대기

1. 실제 저장 이벤트의 서로 다른 created 최소2개를 화면 KST 및 접근 가능한 원본 UTC와 대조한다. UTC/KST 날짜 경계는 격리 fixture로 확인하고 실제 시각이 대시나 현재시각으로 대체되지 않아야 한다.
2. canonical UTC ISO·실제 달력 날짜를 검증한다. malformed/불가능한 날짜/누락·잘못된 형식은 대시이며 Invalid Date/가짜 현재시각/원본 잘못된 값 노출이 없어야 한다.
3. 서버 원본 배열의 순서·최대80개·전역 범위·유형·메시지를 유지하며 상태조회/표시로 이벤트·명령·모델을 변경하지 않는다. 원본 UTC 정보는 접근 가능하고 메시지는 텍스트로만 표현하며 기존 비밀 비노출 경계를 유지한다.
4. 실제 브라우저·좁은 화면·기존 가이드/로그 회귀와 정확한 후보 이미지의 k3s 표시를 확인한다. 동일 백업의 현재1.6 및 하위1.5 복원과 기존기능 회귀 후 안정 승격한다. 하위1.5에 새 시각 수정이 있다고 주장하지 않는다. 최종 영상/PPT·시간 게이트는 별도다.
...
### UI-13 명령 목록 조회 상태 (IDEA011 개정1 / 제품1.7.0 후보)

Commands의 서버 최근 명령 목록에 최초 조회 중·성공한 빈 목록·조회 실패를 구분한다. 이전 행이 있어도 이 RTU의 명령 목록 갱신 실패를 항상 표시하고 마지막 확인값으로 보존한다. 마지막 성공 시각은 브라우저가 성공한 목록 응답을 확인한 시각이며 서버 이벤트 시각이나 명령 완료 시각이 아니다. 전체 SSE 연결 상태와 별개로 안내한다. 다음 정상조회는 새 목록/성공시각을 반영하고 경고를 해제한다.

RTU 변경 시 행·시각·오류를 초기화하고 늦은 A 응답은 B에 반영하지 않는다. 같은 목록의 poll 요청을 겹치지 않게 하고 요청·응답본문 대기에 유한 제한과 abort 수명을 둔다. RTU 변경/컴포넌트 해제 시 소유 요청과 타이머를 정리한다. 기존 조회 주기를 유지하되 이전 요청이 끝나기 전에 새 요청을 누적하지 않는다. 오류 원문을 무제한 노출하지 않고 해당 목록 범위를 명시한다. 접근 가능한 상태/오류 안내와 좁은 화면 가독성을 유지한다.

범위는 GET 목록 표시뿐이다. POST·제어 재시도·서버/DB/API·정렬/최근목록 계약·내보내기 동작·기존 command receipt 및 전체 SSE를 변경하지 않는다. 기존 stable1.6/운영checkpoint180fa68/runtime331/9bd 복구기준과 원래 동결·마감을 보존한다.

### AT-COMMAND-LIST-01 인수 — 구현·검증 대기

1. 실제 목록의 최초 loading, 성공empty, 최초failure를 구분한다. 기존행 뒤 GET실패는행보존+목록특정경고+마지막성공브라우저시각을 표시하고 성공복귀시 새목록/시각·경고해제를 확인한다. SSE정상이어도 목록실패는보여야 한다.
...
## 배포 후 추가 요구 — HTTP 도메인 접근 (PRD1.16 / 제품1.7.2)

사용자 추가 요청에 따라 `charles-k3s`의 기존 NGINX Ingress를 통해 `http://grid.koh.it.kr`로 접근한다. GRID TLS 인증서와 HTTPS 자동 리다이렉트는 구성하지 않는다. 공유 컨트롤러의 다른 서비스 HTTPS 설정은 변경하지 않는다.

- HTTP01: 웹 화면·정적 파일·상태 API·실시간 SSE가 동일 도메인으로 동작한다. 관리 API 인증은 유지한다.
- HTTP02: 기존 default-deny NetworkPolicy에 NGINX 컨트롤러의 TCP3001 접근만 허용하며 MQTT는 공개하지 않는다.
- HTTP03: 일반 HTTP 브라우저에서도 제어 명령 UUID를 생성하고 연동 코드 복사를 처리한다. 복사가 브라우저 정책으로 거절되면 직접 복사 안내를 표시한다.
- 이번 추가 배포는 원래 14시간 실행 및 최종1.7.1 영상·백업·불변 릴리스 증거와 구분한다. DB·MQTT·제어 계약 변경은 없다.
