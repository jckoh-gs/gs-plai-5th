...
이 기록은 전체 목표 완료 선언이 아니다. 현재 기능 기준은 제품1.1.0 / PRD1.8 / runtime8599ab9, stable-runtime-v1.1.0-8599ab9이다. source·실행 이미지·백업·검증의 연결은 checkpoint-1.1.0.json과 run.json에 있다. 이전1.0.0도 보존했다.
...
현재 main 포트포워딩은3104/18884이며 kubectl은 항상 charles-k3s/gs-plai-5h를 명시한다. 터널 종료는 배포 종료와 다르다. 실제 배포는 단일 writer, Ready2/2 컨테이너다. read-only soak은 artifacts/soak/v1.1.0에10초마다 상태와 실제MQTT 샘플을 기록한다. 변경 전후 관찰을 이어 붙여 무중단으로 주장하지 않는다. 1.0관찰은 artifacts/checkpoints/soak-1.0.0에 별도로 보존했다.
