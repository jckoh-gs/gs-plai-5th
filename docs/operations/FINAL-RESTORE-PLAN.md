...
준비 문서이며 최종창 실행 증거가 아니다. 현재 기능상 안정 기준은 **제품1.7.1 / PRD1.15 / runtime40b9ed8 / image234ec56**이다. REVIEW045는 main UI·MQTT·outbox 및 동일1075081216바이트/SHA5ff34 백업의 현재1.7.1·이전1.7 복원26보고서+2실제Pod 식별을 대조했다. root는 양쪽0replicas/0Pods·4BoundPVC·터널 종료와 원래5단지 전체 설정·실행·데이터·시나리오를 확인했다. 선택 메타데이터는 `deploy/verification/stable-backup-40b9ed8.json`, SHA `5ff34f3e5dbc52f07b899f0c8013db130e5fd13c3d315e92aa512d474ea1f782`이다. 불변 `checkpoint-1.7.1.json`은 source `b6c3cbc5742b095bedb32b5f4dea598e685412f5`, SHA `0c9e5bec0a0ef506884b70374aae791e2cb2e38321dcdcd841c10a02c6962ebf`이며 REVIEW046에서2033개 해시·실제Ready·백업 검증을 통과했다. stable-v1.7.1은 운영·근거c9b43bd, stable-runtime-v1.7.1-40b9ed8은 정확 앱40b를 가리키며 원격 refs를 확인했다. 태그 증거와 최신 선택은 run.json.stableCheckpoint를 확인한다.
...
`client.endAsync(true)`에 따른 계획 종료도 close 이벤트와 연결 해제 횟수 증가를 남길 수 있다. 마지막 poll과 종료 파일의 실제 시각·종료 사유를 대조하며 이를 새 네트워크 장애로 단정하지 않는다. 이전 ISSUE030의 API오류1·audit 연결오류1 및4 RTU 수신 불연속/144표본 위치 보존 근거는 지우지 않는다. 이후 ISSUE032의20:19Z 자동 복구에서 audit 연결오류가2로 증가했으므로, 과거 숫자를 최종값으로 복사하지 않고 최신 사건 기록과 실제 종료 카운터를 대조한다. 이어진 ISSUE033의21:08Z 사건에서는 누적 API오류10·primary 연결오류61·audit 연결오류64·연결4/해제3 및 각 RTU 새60표본 위치의 관측 공백이 기록됐다. 별도 읽기 조회로300위치의 같은 run 영속·broker ACK를 확인했지만 구독 관측 공백이나 근본 원인 미확정을 지우지 않는다. 최종 오류 수가 달라졌다면 계획 종료만으로 설명되는지 개별 확인한다. 관측 자료는 수신 범위의 증거이며 무중단 운영·모든 구독자 수신·외부 VPP 수신을 증명하지 않는다.
...
- 23~30분: 인도 해시/상대링크/소스/검수영수증 검사 및 제한 사항 기록. 이는 예상 예산이지30분 완료 보장이 아니다. 7분까지 새 백업 복원 증명이 끝나지 않으면 그 시점의 검증된 stable 체크포인트(현재1075081216바이트/5ff34의1.7.1)를 선택하고 새 백업은 추가 백업으로만 기록하는 대안을 메인이 판단한다. 최종 매니페스트의 backup은 반드시 실제 증명된 선택 스냅샷이어야 한다. 해당 stable의 정확한 snapshot 복원 근거가 없으면 버전명이나 파일 존재로 검증을 추론하지 않는다. 이전1.5/ad34·1.4/be09·1.3/b515ef26 백업은 역사적 fallback이며 현재1.7.1/5ff 증거와 혼합하지 않는다.
