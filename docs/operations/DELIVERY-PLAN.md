...
최종 기상 조작 관측의 `source/weather-observations.jsonl` 및 원래 실행 종료 기록을 같은 영상의 검수와 연결한다. 읽기 전용 `observe-demo-state.py`는 recorder의 보존 소스 목록과 인도 검사의 필수 소스에 추가됐다. 단순 파일 존재·관측 exit0은 출력 변화나 전체 녹화 완료를 증명하지 않는다. 실제 소유RTU/입력/출력·오류행·영상 시각·캡처와 SHA를 함께 대조한다.
...
[남은20개 인수 게이트 준비목록](../product/final-pending-gates.json)은 현재 acceptance의 partial2/pending18과 정확히 대조한 실행 참고자료다. 각ID의 실제 판정·예정 근거·담당을 연결하며 파일 존재나 이 목록 자체를 인수 증거로 사용하지 않는다. 최종 판정 때 기록된 경로를 실제 산출물·원래 성공 종료·해시·직접 검수와 대조한다. 준비 음성161.637초와 장면 하한을 포함한 최소계획290초를 구분하며 실제 최종 영상 길이는 새로 측정한다.
