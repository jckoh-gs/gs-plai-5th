...
설정을 생략한 런타임 기본 포트는 앱 3001, MQTT 1883입니다. 이 신규 작업의 `.env.example`과 로컬 Compose 예제는 기존 서비스와 충돌을 피하기 위해 앱 3101, MQTT 18883을 사용합니다. 아래 빠른 시작은 예제 `.env`를 기존 파일을 덮어쓰지 않고 준비하여 두 포트를 맞춥니다. 기존 설정을 보존한 경우 해당 PORT/MQTT_URL을 따릅니다. 원격 배포의 관리 API는 Bearer 인증이 필요하며 UI 로그인으로 토큰을 입력합니다. 실제 VPP/AWS 연결 완료를 뜻하지 않습니다.
...
`npm run vpp:monitor`는 `status` 토픽에서 `kind: "status"`와 boolean `online`을 출력합니다. `false`는 그대로 보존하고 누락·null·문자열·숫자는 `online: null`로 표시합니다. 명령의 `status`(accepted/completed 등)와 별개입니다. retained online 및 LWT offline은 브로커가 보관하거나 전달한 마지막 상태이며, 구독자의 연결 단절 중 최신 상태 수신이나 현재 장치 건강을 보증하지 않습니다. 기존 허용 필드·비밀 제거·RTU 선택 및 retained 명령결과 거부 규칙은 유지합니다.
